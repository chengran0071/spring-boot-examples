 ...
 *
 *      当WebSocket 与新客户端已成功握手完成，通过写入信息到 ChannelGroup
 *      中的 Channel 来通知所有连接的客户端，然后添加新 Channel 到 ChannelGroup
 *
 *      如果接收到 TextWebSocketFrame，调用 retain() ，并将其写、刷新到 ChannelGroup，
 *      使所有连接的 WebSocket Channel 都能接收到它。和以前一样，retain() 是必需的，
 *      因为当 channelRead0（）返回时，TextWebSocketFrame 的引用计数将递减。
...
public class TextWebSocketFrameHandler extends
    ...
    /**
     * 1.TextWebSocketFrameHandler 继承自 SimpleChannelInboundHandler，
